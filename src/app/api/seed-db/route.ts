...
// This endpoint seeds the production database with demo data
// It should only be used once and then removed or secured
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      userCount,
      schoolCount,
      message: userCount === 0 ? 'Database is empty - needs seeding' : 'Database contains data'
    })
  } catch (error) {
    return NextResponse.json(
